...
volatile unsigned char system_flag_bool = 0; // 系统开关状态记录变量, 0=关机，1=开机
...
const DOWNLOAD_CMD_S download_cmd[] =
    {
        ...
        {DPID_UP, DP_TYPE_BOOL},
        {DPID_DOWN, DP_TYPE_BOOL},
        {DPID_LEFT, DP_TYPE_BOOL},
        {DPID_RIGHT, DP_TYPE_BOOL},
        {DPID_WARNING, DP_TYPE_ENUM},
        {DPID_TIMES, DP_TYPE_ENUM},
};
...
void uart_transmit_output(unsigned char val)
{
    ...
    */
    HAL_UART_Transmit(&huart3, (uint8_t *)&val, 1, 100);
}
/******************************************************************************
...
void all_data_update(void)
{
    ...
    mcu_dp_bool_update(DPID_SWITCH, g_power_switch);                    // BOOL型数据上报;
    mcu_dp_bool_update(DPID_SWITCH_GO, g_clean_switch);                 // BOOL型数据上报;
    mcu_dp_enum_update(DPID_MODE, g_work_mode);                         // 枚举型数据上报;
    mcu_dp_enum_update(DPID_STATUS, g_work_status);                     // 枚举型数据上报;
    mcu_dp_value_update(DPID_BATTERY_PERCENTAGE, g_battery_percentage); // VALUE型数据上报;
    mcu_dp_bool_update(DPID_UP, g_forward);                             // BOOL型数据上报;
    mcu_dp_bool_update(DPID_DOWN, g_backward);                          // BOOL型数据上报;
    mcu_dp_bool_update(DPID_LEFT, g_left);                              // BOOL型数据上报;
    mcu_dp_bool_update(DPID_RIGHT, g_right);                            // BOOL型数据上报;
    mcu_dp_enum_update(DPID_WARNING, g_warning_info);                   // 枚举型数据上报;
}
...
static unsigned char dp_download_switch_handle(const unsigned char value[], unsigned short length)
{
    ...
    if (switch_1 == 0)
    {
        ...
        system_flag_bool = 0;
        g_power_switch = 0;
        g_work_status = STATUS_STANDBY;
        /* 关机时主动停止所有模式，防止 MotorCtrl_Task 继续拉起抽水泵造成振荡抖动 */
        g_auto_enable = 0;
        g_wall_enable = 0;
        UsrTime_Stop();
        MotorCtrl_StopAll();
        mcu_dp_enum_update(DPID_STATUS, g_work_status);
        printf("main rev bt dp:system state: off, system_flag_bool: %d\r\n", system_flag_bool);
    }
    else
    {
        // bool on
        system_flag_bool = 1;
        g_power_switch = 1;
        g_work_status = STATUS_PAUSED;
        mcu_dp_enum_update(DPID_STATUS, g_work_status);
        printf("main rev bt dp:system state: on, system_flag_bool: %d\r\n", system_flag_bool);
    }
...
static unsigned char dp_download_switch_go_handle(const unsigned char value[], unsigned short length)
{
    ...
    if (g_motor_selftest_busy)
    {
        printf("ignore bt dp:switch_go (selftest)\r\n");
        return ERROR;
    }
    ...
    if (switch_go == 0)
    {
        // bool off
        g_clean_switch = 0;
        MotorCtrl_WaterPumpEnable(0); /* 关闭抽水电机 */
        printf("main rev bt dp:switch_go off\r\n");
    }
    else
    {
        // bool on
        g_clean_switch = 1;
        MotorCtrl_WaterPumpEnable(1); /* 使能抽水电机 */
        printf("main rev bt dp:switch_go on\r\n");
    }
    ...
    if (ret == SUCCESS)
    {
        printf("report bt dp:switch_go %d success\r\n", switch_go);
        return SUCCESS;
    }

    else
    {
        printf("report bt dp:switch_go %d failed\r\n", switch_go);
        return ERROR;
    }
}
/*****************************************************************************
...
static unsigned char dp_download_mode_handle(const unsigned char value[], unsigned short length)
{
    ...
    if (g_motor_selftest_busy)
    {
        printf("ignore bt dp:mode (selftest)\r\n");
        return ERROR;
    }
    ...
    case 2: /* 自动清扫模式 */
        ...
        if (!g_auto_enable)
        {
            UsrTime_Stop();      /* 手动关闭自动模式时取消待定定时，防止到期回调误触发 */
            MotorCtrl_StopAll(); // 关闭自动模式时停机
        }
        g_work_status = g_auto_enable ? STATUS_SMART_CLEAN : STATUS_STANDBY;
        mcu_dp_enum_update(DPID_STATUS, g_work_status);
        printf("main rev bt dp:mode 2, g_auto_enable = %d\r\n", g_auto_enable);
        break;
    ...
    case 3: /* 爬壁模式 */
        ...
        if (!g_wall_enable)
        {
            UsrTime_Stop();      /* 手动关闭爬壁模式时取消待定定时，防止到期回调误触发 */
            MotorCtrl_StopAll(); // 关闭爬墙模式时停机
        }
        g_work_status = g_wall_enable ? STATUS_WALL_CLEAN : STATUS_STANDBY;
        mcu_dp_enum_update(DPID_STATUS, g_work_status);
        printf("main rev bt dp:mode 3, g_wall_enable = %d\r\n", g_wall_enable);
        break;
...
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_up_handle(const unsigned char value[], unsigned short length)
{
    // 示例:当前DP类型为BOOL
    unsigned char ret;
    // 0:off/1:on
    ...
    if (g_motor_selftest_busy)
    {
        printf("ignore bt dp:up (selftest)\r\n");
        return ERROR;
    }

    up = mcu_get_dp_download_bool(value, length);
    if (up == 0)
    {
        // bool off
        g_forward = 0;
        MotorCtrl_Stop();
        printf("main rev bt dp:up off\r\n");
    }
    else
    {
        // bool on
        g_forward = 1;
        MotorCtrl_SetAction(MOTOR_CTRL_FORWARD, 999, 0);
        printf("main rev bt dp:up on\r\n");
    }

    // There should be a report after processing the DP
    ret = mcu_dp_bool_update(DPID_UP, up);
    if (ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_down_handle
功能描述 : 针对DPID_DOWN的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_down_handle(const unsigned char value[], unsigned short length)
{
    // 示例:当前DP类型为BOOL
    unsigned char ret;
    // 0:off/1:on
    ...
    if (g_motor_selftest_busy)
    {
        printf("ignore bt dp:down (selftest)\r\n");
        return ERROR;
    }

    down = mcu_get_dp_download_bool(value, length);
    if (down == 0)
    {
        // bool off
        g_backward = 0;
        MotorCtrl_Stop();
        printf("main rev bt dp:down off\r\n");
    }
    else
    {
        // bool on
        g_backward = 1;
        MotorCtrl_SetAction(MOTOR_CTRL_BACKWARD, 999, 0);
        printf("main rev bt dp:down on\r\n");
    }

    // There should be a report after processing the DP
    ret = mcu_dp_bool_update(DPID_DOWN, down);
    if (ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_left_handle
功能描述 : 针对DPID_LEFT的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_left_handle(const unsigned char value[], unsigned short length)
{
    // 示例:当前DP类型为BOOL
    unsigned char ret;
    // 0:off/1:on
    ...
    if (g_motor_selftest_busy)
    {
        printf("ignore bt dp:left (selftest)\r\n");
        return ERROR;
    }

    left = mcu_get_dp_download_bool(value, length);
    if (left == 0)
    {
        // bool off
        g_left = 0;
        MotorCtrl_Stop();
        printf("main rev bt dp:left off\r\n");
    }
    else
    {
        // bool on
        g_left = 1;
        MotorCtrl_SetAction(MOTOR_CTRL_TURN_LEFT, 999, 0);
        printf("main rev bt dp:left on\r\n");
    }

    // There should be a report after processing the DP
    ret = mcu_dp_bool_update(DPID_LEFT, left);
    if (ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_right_handle
功能描述 : 针对DPID_RIGHT的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_right_handle(const unsigned char value[], unsigned short length)
{
    // 示例:当前DP类型为BOOL
    unsigned char ret;
    // 0:off/1:on
    ...
    if (g_motor_selftest_busy)
    {
        printf("ignore bt dp:right (selftest)\r\n");
        return ERROR;
    }

    right = mcu_get_dp_download_bool(value, length);
    if (right == 0)
    {
        // bool off
        g_right = 0;
        MotorCtrl_Stop();
        printf("main rev bt dp:right off\r\n");
    }
    else
    {
        // bool on
        g_right = 1;
        MotorCtrl_SetAction(MOTOR_CTRL_TURN_RIGHT, 999, 0);
        printf("main rev bt dp:right on\r\n");
    }

    // There should be a report after processing the DP
    ret = mcu_dp_bool_update(DPID_RIGHT, right);
    if (ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/**
 * @brief 定时清扫到期回调：在主循环 UsrTime_Poll() 中执行，停机并向 App 上报状态。
 * @note  禁止在中断中调用。
 */
static void on_clean_timer_expired(UsrTimeTask_t task)
{
    (void)task;
    /* 1. 先清模式标志，防止 MotorCtrl_Task 在本轮或下轮重新拉起电机 */
    g_auto_enable = 0;
    g_wall_enable = 0;
    /* 2. 直接停机：无论 MotorCtrl_Task 是否因竞态错过了 IsExpired，
          此处保证 PWM/GPIO 寄存器被清零，电机物理停转 */
    MotorCtrl_StopAll();
    /* 3. 更新 App 显示状态 */
    g_work_status = STATUS_STANDBY;
    mcu_dp_enum_update(DPID_STATUS, g_work_status);
    printf("clean timer expired -> all motors stopped\r\n");
}
/*****************************************************************************
函数名称 : dp_download_times_handle
功能描述 : 针对DPID_TIMES的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 只下发类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_times_handle(const unsigned char value[], unsigned short length)
{
    // 示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char times;
    uint8_t use_wall_mode = 0u;

    times = mcu_get_dp_download_enum(value, length);
    g_clean_time = times;

    if (g_motor_selftest_busy)
    {
        printf("ignore bt dp:times (selftest busy)\r\n");
        return ERROR;
    }
    ...
    /* 定时跟随当前工作模式：
       - 当前处于爬壁模式时，保持爬壁并启动定时
       - 其他情况默认按自动清扫模式启动定时 */
    if (g_wall_enable || (g_work_mode == 3u))
    {
        use_wall_mode = 1u;
    }
    ...
    if (use_wall_mode)
    {
        g_auto_enable = 0;
        g_wall_enable = 1;
        g_work_status = STATUS_WALL_CLEAN;
    }
    else
    {
        g_auto_enable = 1;
        g_wall_enable = 0;
        g_work_status = STATUS_SMART_CLEAN;
    }

    mcu_dp_enum_update(DPID_STATUS, g_work_status);
    UsrTime_SetExpiredCallback(on_clean_timer_expired);

    switch (times)
    {
    case 0:
        UsrTime_Start30Min();
        printf("main rev bt dp:times 0(30 min OK)\r\n");
        break;

    case 1:
        UsrTime_Start1Hour();
        printf("main rev bt dp:times 1(1 hour OK)\r\n");
        break;

    case 2:
        UsrTime_Start2Hour();
        printf("main rev bt dp:times 2(2 hour OK)\r\n");
        break;

    case 3:
        UsrTime_Start3Hour();
        printf("main rev bt dp:times 3(3 hour OK)\r\n");
        break;

    case 4:
        UsrTime_Start5Hour();
        printf("main rev bt dp:times 4(5 hour OK)\r\n");
        break;

    case 5:
        UsrTime_Start8Hour();
        printf("main rev bt dp:times 5(8 hour OK)\r\n");
        break;

    case 6:
        // UsrTime_Start1MinTest();
        UsrTime_StartSeconds(10);
        printf("main rev bt dp:times 6(1 min Test OK)\r\n");
        break;

    default:
        printf("main rev bt dp:times error\r\n");
        break;
    }

    // There should be a report after processing the DP
    ret = mcu_dp_enum_update(DPID_TIMES, times);
    if (ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid, const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    case DPID_UP:
        // 前进处理函数
        ret = dp_download_up_handle(value, length);
        break;
    case DPID_DOWN:
        // 后退处理函数
        ret = dp_download_down_handle(value, length);
        break;
    case DPID_LEFT:
        // 向左清扫处理函数
        ret = dp_download_left_handle(value, length);
        break;
    case DPID_RIGHT:
        // 向右清扫处理函数
        ret = dp_download_right_handle(value, length);
        break;
    case DPID_TIMES:
        // 清扫时间处理函数
        ret = dp_download_times_handle(value, length);
        break;

...
******************************************************************************/
#define PRODUCT_KEY "h9koocv2" // 开发平台创建产品后生成的16位字符产品唯一标识
...
// 工作模式(可下发可上报)
// 备注:【必选】枚举值按需选择，不需要的可删除，可以调整排列顺序，但是单词必须一模一样，不可修改单词，例如不能把smart修改为AOTU。
// standby 待机
// random 随机模式
// smart 自动模式
// wall_follow 沿墙模式
...
#define DPID_MODE 3
// 工作状态(只上报)
// 备注:【必选】默认枚举值不可删除，否则影响控制面板逻辑，可以调整排列顺序，但是单词必须一模一样，不可修改单词，例如不能把smart修改为AOTU，保证默认的不变，可在后面增加其他状态值，状态值尽量是状态的英文小写单词，如需要增加故障中状态，则为in_trouble。
// standby 待机中
// smart_clean 自动清扫中
// wall_clean 沿墙清扫中
// spot_clean 定点清扫中
...
// 剩余电量(只上报)
// 备注:
#define DPID_BATTERY_PERCENTAGE 6
// 前进(可下发可上报)
// 备注:
#define DPID_UP 101
// 后退(可下发可上报)
// 备注:
#define DPID_DOWN 102
// 向左清扫(可下发可上报)
// 备注:
#define DPID_LEFT 103
// 向右清扫(可下发可上报)
// 备注:
#define DPID_RIGHT 104
// 报警信息(只上报)
// 备注:
#define DPID_WARNING 105
// 清扫时间(只下发)
// 备注:
#define DPID_TIMES 106
